...
class NX2Table(atpy.Table):
    ...
    def fill_nans(self, column):
        ...
        if column == 'TIME':
            ...
            func = scipy.interpolate.interp1d(self.TIME[
                                              index], self[column][index], bounds_error=False)
    ...
    def plot_course(self, scale=50, n=300):
        ...
        quiver_wind = ax.quiver(self.x[::n], self.y[::n], self.TWS[::n]*np.sin(wind_ang[
                                ::n]/180.*np.pi), self.TWS[::n]*np.cos(wind_ang[::n]/180.*np.pi), scale=scale, color='g')
        quiver_bsp = ax.quiver(self.x[::n], self.y[::n], self.BSP[::n]*np.sin(self.HDC[
            ::n]/180.*np.pi), self.BSP[::n]*np.cos(self.HDC[::n]/180.*np.pi), scale=scale)
        quiver_sog = ax.quiver(self.x[::n], self.y[::n], self.SOG[::n]*np.sin(self.COG[
            ::n]/180.*np.pi), self.SOG[::n]*np.cos(self.COG[::n]/180.*np.pi), scale=scale, color='b')
        if scale == None:
            qk_scale = None
        else:
            qk_scale = scale/20.
        ...
        qk_bsp = ax.quiverkey(
            quiver_bsp, .1, 0.9, qk_scale, 'Fahrt durchs Wasser (ohne Drift)', labelpos='E')
    ...
    def add_rowing_old_format(self, filename, verbose=True):
        ...
        '''
        if verbose:
            print 'Input data does not contain info on month and year.'
        rowdata = atpy.Table(filename, type='ascii', delimiter=';')
        # rowtime = np.array(map(lambda x:
        # datetime.datetime(self.read_date[2],self.read_date[1],*x),
        # zip(rowdata['Tag'], rowdata['Stunde'], rowdata['Minute'])))
        ...
        for i in range(len(rowdata)):
            ind = (self.day == rowdata.Tag[i]) & (self.hour == rowdata.Stunde[
                                                  i]) & (self.minute == rowdata.Minute[i])
            # ind = (self.datetime() >= rowtime[i]) & (self.datetime() <= (rowtime[i]
...
Fragen an: Moritz.guenther@hs.uni-hamburg.de</description>
    ...
    <Style id="redLine">
      ...
      <open>0</open>  ''')
                phases = [list(g) for key, g in itertools.groupby(np.arange(
                    len(self)), lambda k: (self.sailing[k] == 1.)) if key == False]
                for phase in phases:
                    write_leg(
                        self, kmlFile, phase, name='Ruderstrecke', style='#yellowLine', skip=1)
                ...
                phases = [list(g) for key, g in itertools.groupby(np.arange(
                    len(self)), lambda k: (self.sailing[k] == 1.)) if key == True]
                for phase in phases:
                    write_leg(
                        self, kmlFile, phase, name='Segelstrecke', style='#redLine', skip=1)
                kmlFile.write('    </Folder>')
            else:
...
def remove_Danube_current(data):
    ...
    schwaller = readsav(os.path.join(os.path.dirname(
        __file__), 'data', 'stromgeschwindigkeit.sav'))['strom']
